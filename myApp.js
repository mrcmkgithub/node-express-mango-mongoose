...
var createAndSavePerson = function(done) {
  var person = new Person({name:"Ahmet",age:34,favoriteFoods:["beer","pizza"]});
  person.save(function(err,data){
  if(err){
  return done(err);
  }
...
var removeById = function(personId, done) {
  Person.findById(personId,function(err,data){
  if(err)return done(err);
  Person.deleteOne({_id:personId},function(err){
  if(err)return done(err);
    done(null,data);
  });      
  });  
...
var queryChain = function(done) {
  ...
  Person.find({favoriteFoods:foodToSearch}).sort({name:1}).limit(2).select("name favoriteFoods").exec(function(err,data){
  if(err)return done(err);
    done(null,data);
  });  
};
